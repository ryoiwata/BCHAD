...
// AssemblePRActivity creates a GitHub branch and pull request for the pipeline run.
// It requires GITHUB_TOKEN to be set in the worker's environment.
// If GITHUB_TOKEN is unset it falls back to a stub URL (development only).
func AssemblePRActivity(ctx context.Context, input PRInput) (*PROutput, error) {
	slog.Info("activity: assemble PR",
		"run_id", input.RunID,
	...
	if githubToken == "" {
		slog.Warn("activity: GITHUB_TOKEN not set — returning stub PR URL")
		repoURL := input.RepoURL
		if repoURL == "" {
			repoURL = fmt.Sprintf("https://github.com/athena-digital/%s", input.ProductID)
		}
		return &PROutput{
			PRURL:      fmt.Sprintf("%s/pull/stub-no-token", repoURL),
			BranchName: branchName,
		}, nil
	}

	if input.RepoURL == "" {
		return nil, fmt.Errorf("assemble PR: RepoURL is empty — index the repo first")
	}

	owner, repo, err := parseGitHubRepo(input.RepoURL)
	if err != nil {
		return nil, fmt.Errorf("assemble PR: %w", err)
	}

	gh := newGHClient(githubToken)

	// Resolve default branch and its HEAD SHA.
	repoInfo, err := gh.getRepo(ctx, owner, repo)
	if err != nil {
		return nil, fmt.Errorf("assemble PR: get repo: %w", err)
	}
	headSHA, err := gh.getBranchSHA(ctx, owner, repo, repoInfo.DefaultBranch)
	if err != nil {
		return nil, fmt.Errorf("assemble PR: get %s sha: %w", repoInfo.DefaultBranch, err)
	}

	// Create the branch (no-op if it already exists).
	if err := gh.createBranch(ctx, owner, repo, branchName, headSHA); err != nil {
		return nil, fmt.Errorf("assemble PR: create branch %s: %w", branchName, err)
	}

	// Commit a pipeline manifest file so the branch has at least one commit ahead.
	...
	filePath := fmt.Sprintf(".bchad/%s.json", input.PlanID)
	commitMsg := fmt.Sprintf("chore(bchad): scaffold pipeline run %s", input.RunID)
	if err := gh.createFile(ctx, owner, repo, filePath, commitMsg, string(metaJSON), branchName); err != nil {
		return nil, fmt.Errorf("assemble PR: create manifest commit: %w", err)
	}

	// Create the pull request.
	prTitle := fmt.Sprintf("BCHAD: %s [%s]", input.PlanID, input.ProductID)
	prBody := fmt.Sprintf(
		"Generated by BCHAD pipeline run `%s`.\n\n**Stages:** %s\n\n"+
			"> Stage-generated code will be added in subsequent commits as Phase 3 activities are implemented.",
		input.RunID,
		strings.Join(input.StageIDs, ", "),
	)
	prURL, err := gh.createPR(ctx, owner, repo, branchName, repoInfo.DefaultBranch, prTitle, prBody)
	if err != nil {
		return nil, fmt.Errorf("assemble PR: create pr: %w", err)
	}

	slog.Info("activity: PR created", "run_id", input.RunID, "pr_url", prURL)
	return &PROutput{
		PRURL:      prURL,
		BranchName: branchName,
	}, nil
